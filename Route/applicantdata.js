...
//   host: "91.204.209.39", 
//   user: "ntechabp_skywayapidb",
//   password: "0966202667n",
//   database: "ntechabp_skywayapi_db"
// }).promise();
...
//     const imageData = imageEntry.get({ plain: true });

//     // Construct URLs for the images and video using HTTPS
//     const baseUrl = `https://${req.get('host')}/applicantimagetest/`; // Changed to https
//     const responseData = {
//       ...imageData,
//       personalImageUrl: imageData.personalimage ? `${baseUrl}${imageData.personalimage}` : null,
//       fullBodyImageUrl: imageData.fullbodyimage ? `${baseUrl}${imageData.fullbodyimage}` : null,
//       passportImageUrl: imageData.passportimage ? `${baseUrl}${imageData.passportimage}` : null,
//       videoUrl: imageData.video ? `${baseUrl}${imageData.video}` : null,
//     };
...
const pool = mysql.createPool({
  host: "91.204.209.39", 
  user: "ntechabp_alwasitapi_user",
  password: "123456@nuru",
  database: "ntechabp_alwasit"
}).promise();

// Multer storage configuration
const storage = multer.diskStorage({
  destination: (req, file, cb) => {
    const dir = path.join(__dirname, "../applicantimagetest/");
...
router.post("/tupload-image", upload.fields([
  ...
  try {
    // Check if passportnum already exists
    const [existingEntry] = await pool.query("SELECT * FROM testapplicantimg WHERE passportnum = ?", [req.body.passportnum]);

    if (existingEntry.length > 0) {
      return res.status(400).json({ status: "error", message: "Passport number already exists." });
    }
    ...
    const newImageEntry = {
      ...
      cooking: req.body.cooking === "true",
      eldercare: req.body.eldercare === "true",
      childerncare: req.body.childerncare === "true",
      ironingclothes: req.body.ironingclothes === "true",
      ...
      sponsorId: req.body.sponsorId,
      sponsorAddress: req.body.sponsorAddress,
      sponsorCity: req.body.sponsorCity,
      nationalId: req.body.nationalId,
      email: req.body.email,
      sponsorName: req.body.sponsorName,
      sponsorPhone: req.body.sponsorPhone,
...
//     // Construct URLs for the images and video
//     const baseUrl = `${req.protocol}://${req.get('host')}/applicantimagetest/`;
//     const responseData = {
//       ...imageEntry,
//       personalImageUrl: imageEntry.personalimage ? `${baseUrl}${imageEntry.personalimage}` : null,
//       fullBodyImageUrl: imageEntry.fullbodyimage ? `${baseUrl}${imageEntry.fullbodyimage}` : null,
//       passportImageUrl: imageEntry.passportimage ? `${baseUrl}${imageEntry.passportimage}` : null,
//       videoUrl: imageEntry.video ? `${baseUrl}${imageEntry.video}` : null,
//     };
